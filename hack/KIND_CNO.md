...
Additionally, a user may want to specify which version of OpenShift container images to build from. This is by default set to use
the latest available container image. However by specifying a tag the version can be overriden:

```
$ OCP_IMAGE_VER=4.6 ovn-kind-cno.sh
```
