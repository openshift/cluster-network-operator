...
if ( ! ( command -v controller-gen > /dev/null )  || test "$(controller-gen --version)" != "Version: ${LATEST_CONTROLLER_GEN_VER}" ); then
  ...
  builddir="$(mktemp -d)"
  cd "${builddir}"
  GO111MODULE=on GOFLAGS="" go install sigs.k8s.io/controller-tools/cmd/controller-gen@${LATEST_CONTROLLER_GEN_VER}
  cd "${olddir}"
...
echo "${HEADER}" > manifests/0000_70_cluster-network-operator_01-Default.crd.yaml
cat vendor/github.com/openshift/api/operator/v1/0000_70_cluster-network-operator_01-Default.crd.yaml >> manifests/0000_70_cluster-network-operator_01-Default.crd.yaml

echo "${HEADER}" > manifests/0000_70_cluster-network-operator_01-CustomNoUpgrade.crd.yaml
cat vendor/github.com/openshift/api/operator/v1/0000_70_cluster-network-operator_01-CustomNoUpgrade.crd.yaml >> manifests/0000_70_cluster-network-operator_01-CustomNoUpgrade.crd.yaml

echo "${HEADER}" > manifests/0000_70_cluster-network-operator_01-TechPreviewNoUpgrade.crd.yaml
cat vendor/github.com/openshift/api/operator/v1/0000_70_cluster-network-operator_01-TechPreviewNoUpgrade.crd.yaml >> manifests/0000_70_cluster-network-operator_01-TechPreviewNoUpgrade.crd.yaml


echo "${HEADER}" > manifests/0000_70_cluster-network-operator_01_egr_crd.yaml
cat vendor/github.com/openshift/api/networkoperator/v1/001-egressrouter.crd.yaml >> manifests/0000_70_cluster-network-operator_01_egr_crd.yaml
