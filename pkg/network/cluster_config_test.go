...
func TestValidClusterConfigLiveMigration(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"error when cluster network overlaps with ovn-k internal subnet",
			&bootstrap.InfraStatus{StandaloneManagedCluster: true},
			&configv1.Network{
				Spec: networkConfig,
				ObjectMeta: metav1.ObjectMeta{
		...
		},
		{
			...
			&configv1.Network{
				Spec: func() configv1.NetworkSpec {
					cfg := networkConfig
					// default v4 transit subnet is 100.88.0.0/16
					cfg.ServiceNetwork = []string{"100.88.2.0/24"}
					return cfg
				}(),
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{names.NetworkTypeMigrationAnnotation: ""},
			...
			true,
			"network serviceNetwork(100.88.2.0/24) overlaps with network v4InternalTransitSwitchSubnet(100.88.0.0/16)",
		},
		{
		...
		},
		{
			...
			[]crclient.Object{
				...
				&corev1.Pod{
					ObjectMeta: metav1.ObjectMeta{
						Annotations: map[string]string{networkv1.AssignMacvlanAnnotation: ""},
					},
				},
			},
			true,
