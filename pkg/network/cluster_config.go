...
func validateOVNKubernetesCIDROverlap(clusterConfig *configv1.Network, operConfig *operv1.Network) error {
	...
	var subnets []subnet
	for _, cn := range clusterConfig.Spec.ClusterNetwork {
		subnets = append(subnets, subnet{
			name: "clusterNetwork",
			cidr: cn.CIDR,
		})
	}
	for _, svcNet := range clusterConfig.Spec.ServiceNetwork {
		subnets = append(subnets, subnet{
			name: "serviceNetwork",
			cidr: svcNet,
		})
	...
	subnets = append(subnets, subnet{
		...
		cidr: defaultV4TransitSwitchSubnet,
	})
	subnets = append(subnets, subnet{
		name: "v6InternalTransitSwitchSubnet",
		cidr: defaultV6TransitSwitchSubnet,
	})
	subnets = append(subnets, subnet{
		name: "v4InternalMasqueradeSubnet",
		cidr: defaultV4MasqueradeSubnet,
	})
	subnets = append(subnets, subnet{
		name: "v6InternalMasqueradeSubnet",
		cidr: defaultV6MasqueradeSubnet,
	})
