...
func bootstrapOVN(conf *operv1.Network, kubeClient client.Client) (*bootstrap.BootstrapResult, error) {
	...
	if wait.ErrWaitTimeout == err {
		log.Printf("Timeout exceeded while bootstraping OVN, expected amount of control plane nodes (%v) do not match found (%v), continuing deployment with found replicas", controlPlaneReplicaCount, len(masterNodeList.Items))
	} else if err != nil {
