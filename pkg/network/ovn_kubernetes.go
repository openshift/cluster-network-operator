...
func renderOVNKubernetes(conf *operv1.NetworkSpec, bootstrapResult *bootstrap.BootstrapResult, manifestDir string,
	...
	data.Data["DpuHostModeValues"] = bootstrapResult.OVN.OVNKubernetesConfig.DpuHostModeValues
	data.Data["DpuModeLabel"] = bootstrapResult.OVN.OVNKubernetesConfig.DpuModeLabel
	data.Data["DpuModeValues"] = bootstrapResult.OVN.OVNKubernetesConfig.DpuModeValues
	data.Data["SmartNicModeLabel"] = bootstrapResult.OVN.OVNKubernetesConfig.SmartNicModeLabel
	data.Data["SmartNicModeValues"] = bootstrapResult.OVN.OVNKubernetesConfig.SmartNicModeValues
...
// getKeyAndValueFromLabel returns specified label key and value (if any).
...
func getKeyValueFromLabel(label string) (string, []string, error) {
	...
	if len(parts) != 2 {
		return "", nil, fmt.Errorf("invalid label format %s, expected format: key= or key=value", label)
	}
...
func bootstrapOVNConfig(conf *operv1.Network, kubeClient cnoclient.Client, hc *hypershift.HyperShiftConfig, infraStatus *bootstrap.InfraStatus) (*bootstrap.OVNConfigBoostrapResult, error) {
	...
	ovnConfigResult.DpuHostModeLabel, ovnConfigResult.DpuHostModeValues, err = getKeyValueFromLabel(ovnConfigResult.DpuHostModeLabel)
	if err != nil {
	...
	ovnConfigResult.DpuModeLabel, ovnConfigResult.DpuModeValues, err = getKeyValueFromLabel(ovnConfigResult.DpuModeLabel)
	if err != nil {
	...
	ovnConfigResult.SmartNicModeLabel, ovnConfigResult.SmartNicModeValues, err = getKeyValueFromLabel(ovnConfigResult.SmartNicModeLabel)
	if err != nil {
