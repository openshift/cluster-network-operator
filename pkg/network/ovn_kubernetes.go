...
func renderOVNKubernetes(conf *operv1.NetworkSpec, bootstrapResult *bootstrap.BootstrapResult, manifestDir string,
	...
	var renderIPsecDaemonSet, removeIPsecDaemonSet bool
	...
	if c.IPSecExternal.State == operv1.IPSecExternalStateEnabled {
	...
	} else if c.IPSecExternal.State == operv1.IPSecExternalStateDisabled || c.IPSecExternal.State == "" {
		...
		// if client.MachineConfig() != nil {
		// 	klog.Infof("Mac")
		// }
	} else {
		//TODO: remove
		klog.Infof("IPSec: Illegal external value!!!")
	}
...
func bootstrapOVN(conf *operv1.Network, kubeClient cnoclient.Client, infraStatus *bootstrap.InfraStatus) (*bootstrap.OVNBootstrapResult, error) {
	...
	if err := kubeClient.ClientFor("").CRClient().Get(context.TODO(), nsn, ipsecDaemonSet); err != nil {
		...
		ipsecStatus.IsHostIPsecMigration = true
	}

	if ipsecStatus == nil {
		ipsecStatus = &bootstrap.OVNUpdateStatus{}
		// Retrieve IPsec daemonset with name ovn-ipsec-containerized if we are upgrading from 4.14.
		nsn = types.NamespacedName{Namespace: util.OVN_NAMESPACE, Name: "ovn-ipsec-containerized"}
		if err := kubeClient.ClientFor("").CRClient().Get(context.TODO(), nsn, ipsecDaemonSet); err != nil {
			if !apierrors.IsNotFound(err) {
				return nil, fmt.Errorf("Failed to retrieve existing 4.14 ipsec DaemonSet: %w", err)
			} else {
				ipsecStatus = nil
			}
		} else {
	...
	if ipsecStatus == nil {
		ipsecStatus = &bootstrap.OVNUpdateStatus{}
		// Retrieve IPsec daemonset with name ovn-ipsec-host if we are just flapping ipsec config.
