...
func Render(conf *netv1.NetworkConfigSpec, manifestDir string) ([]*uns.Unstructured, error) {
	...
	// render Multus
	o, netAttachDefCRDRendered, err := RenderMultus(conf, manifestDir)
	if err != nil {
		return nil, err
	}
	objs = append(objs, o...)
...
func ValidateMultus(conf *netv1.NetworkConfigSpec) []error {
	...
	if !deployMultus && len(conf.AdditionalNetworks) > 0 {
		return []error{errors.Errorf("additional networks cannot be specified with deploying Multus")}
	}
	return []error{}
}
...
func RenderAdditionalNetworks(conf *netv1.NetworkConfigSpec, manifestDir string, netAttachDefCRDRendered bool) ([]*uns.Unstructured, error) {
	var err error
	ans := conf.AdditionalNetworks
...
func RenderMultus(conf *netv1.NetworkConfigSpec, manifestDir string) ([]*uns.Unstructured, bool, error) {
	...
	// enabling Multus always renders the CRD
	objs, err = renderAdditionalNetworksCRD(manifestDir)
	if err != nil {
