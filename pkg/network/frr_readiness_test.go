...
func TestShouldSkipOVNKUntilFRRReady(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			bootstrapResult: &bootstrap.BootstrapResult{
				OVN: bootstrap.OVNBootstrapResult{
					NodeUpdateStatus: &bootstrap.OVNUpdateStatus{ // ovnk is running
						Name: "ovnkube-node",
					},
				},
			},
		...
		},
		{
			...
			bootstrapResult: &bootstrap.BootstrapResult{
				OVN: bootstrap.OVNBootstrapResult{
					NodeUpdateStatus: nil, // ovnk not running
				},
			},
			endpointSlice: nil,
			expectedSkip:  false,
		...
		{
			name: "OVNK not running, FRR webhook ready - should not skip",
			conf: &operv1.NetworkSpec{
				DefaultNetwork: operv1.DefaultNetworkDefinition{
					Type: operv1.NetworkTypeOVNKubernetes,
					OVNKubernetesConfig: &operv1.OVNKubernetesConfig{
		...
		{
			name: "OVNK not running, FRR webhook not ready - should skip",
			conf: &operv1.NetworkSpec{
				DefaultNetwork: operv1.DefaultNetworkDefinition{
					Type: operv1.NetworkTypeOVNKubernetes,
					OVNKubernetesConfig: &operv1.OVNKubernetesConfig{
		...
		{
			name: "OVNK not running, FRR webhook has empty endpoints - should skip",
			conf: &operv1.NetworkSpec{
				DefaultNetwork: operv1.DefaultNetworkDefinition{
					Type: operv1.NetworkTypeOVNKubernetes,
					OVNKubernetesConfig: &operv1.OVNKubernetesConfig{
