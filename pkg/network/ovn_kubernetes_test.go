...
func TestDpuLeaseConfigRendering(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	bootstrapResult.OVN = bootstrap.OVNBootstrapResult{
		...
		OVNKubernetesConfig: &bootstrap.OVNConfigBoostrapResult{
			...
			SmartNicModeLabel:         OVN_NODE_SELECTOR_DEFAULT_SMART_NIC,
			MgmtPortResourceName:      "",
			DpuNodeLeaseRenewInterval: 15,
			DpuNodeLeaseDuration:      60,
	...
	g.Expect(envVars["OVNKUBE_NODE_LEASE_RENEW_INTERVAL"]).To(Equal("15"))
	g.Expect(envVars["OVNKUBE_NODE_LEASE_DURATION"]).To(Equal("60"))
	_, hasNamespace := envVars["OVNKUBE_NODE_LEASE_NAMESPACE"]
	g.Expect(hasNamespace).To(BeTrue(), "OVNKUBE_NODE_LEASE_NAMESPACE env var should be present")
}

func TestDpuLeaseConfigDefaults(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	envVars := extractDaemonSetEnvVars(g, objs, "ovnkube-node-dpu-host", "ovnkube-controller")
	g.Expect(envVars["OVNKUBE_NODE_LEASE_RENEW_INTERVAL"]).To(Equal("10"))
	g.Expect(envVars["OVNKUBE_NODE_LEASE_DURATION"]).To(Equal("40"))
	_, hasNamespace := envVars["OVNKUBE_NODE_LEASE_NAMESPACE"]
	g.Expect(hasNamespace).To(BeTrue(), "OVNKUBE_NODE_LEASE_NAMESPACE env var should be present")
}

func TestDpuLeaseConfigZeroDisables(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	// Timer=0 should render as "0", not fall back to defaults
	envVars := extractDaemonSetEnvVars(g, objs, "ovnkube-node-dpu-host", "ovnkube-controller")
	g.Expect(envVars["OVNKUBE_NODE_LEASE_RENEW_INTERVAL"]).To(Equal("0"))
	g.Expect(envVars["OVNKUBE_NODE_LEASE_DURATION"]).To(Equal("0"))
}
