...
func TestRenderOVNKubernetes(t *testing.T) {
	...
	g.Expect(ovnkubeScriptLib).To(ContainSubstring("ovn-sbctl -t 5 --inactivity-probe=0 set-connection punix:${sbdb_sock}"))
	g.Expect(ovnkubeScriptLib).To(ContainSubstring(`NORTHD_PROBE_OPTION="options:northd_probe_interval=0"`))
	g.Expect(strings.Count(ovnkubeScriptLib, "--inactivity-probe=0")).To(Equal(2))
	g.Expect(extractOVNKubeConfig(g, objs)).To(ContainSubstring("inactivity-probe=0"))
