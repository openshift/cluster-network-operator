...
func TestRenderOVNKubernetes(t *testing.T) {
	...
	featureGatesCNO := featuregates.NewFeatureGate(
		[]configv1.FeatureGateName{features.FeatureGateAdminNetworkPolicy, features.FeatureGateDNSNameResolver},
		[]configv1.FeatureGateName{features.FeatureGatePersistentIPsForVirtualization},
	)
...
logfile-maxage=0`,
			...
			enabledFeatureGates:    []configv1.FeatureGateName{features.FeatureGateAdminNetworkPolicy},
		},
		{
...
logfile-maxage=0`,
			controlPlaneReplicaCount: 2,
			enabledFeatureGates:      []configv1.FeatureGateName{features.FeatureGateDNSNameResolver},
		},
	}
	g := NewGomegaWithT(t)
	...
	for i, tc := range testcases {
		t.Run(fmt.Sprintf("%d:%s", i, tc.desc), func(t *testing.T) {
			...
			knownFeatureGates := []configv1.FeatureGateName{
				features.FeatureGateAdminNetworkPolicy,
				features.FeatureGateDNSNameResolver,
				features.FeatureGatePersistentIPsForVirtualization,
			}
