...
func TestRenderOVNKubernetes_UDNIsolationModeOverride(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	t.Run("with udn-isolation-mode override", func(t *testing.T) {
		ovnkubeScriptLib := renderWithOverrides(map[string]string{"udn-isolation-mode": "loose"})
		g.Expect(ovnkubeScriptLib).To(ContainSubstring(`--udn-isolation-mode=loose"`))
	})

	t.Run("without udn-isolation-mode override", func(t *testing.T) {
		ovnkubeScriptLib := renderWithOverrides(nil)
		g.Expect(ovnkubeScriptLib).To(ContainSubstring(`--udn-isolation-mode="`))
	})
}
