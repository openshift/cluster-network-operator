...
import (
	...
	mcfghelper "github.com/openshift/machine-config-operator/pkg/apihelpers"
...
func (r *ReconcileOperConfig) ensureMachineConfigPools(ctx context.Context, clusterConfig *configv1.Network, condType string, isMatch match, nowTimestamp metav1.Time) (isDesired, isStable bool, err error) {
	...
	for _, pool := range pools.Items {
		if mcfghelper.IsMachineConfigPoolConditionTrue(pool.Status.Conditions, mcfgv1.MachineConfigPoolDegraded) {
			*condition = metav1.Condition{
				Type:               condType,
				Status:             metav1.ConditionFalse,
	...
	for _, pool := range pools.Items {
		if mcfghelper.IsMachineConfigPoolConditionTrue(pool.Status.Conditions, mcfgv1.MachineConfigPoolUpdating) {
			*condition = metav1.Condition{
				Type:               condType,
				Status:             metav1.ConditionFalse,
