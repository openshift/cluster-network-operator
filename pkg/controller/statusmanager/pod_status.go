...
func (status *StatusManager) SetFromPods() {
	...
	// One-time migration: when upgrading from a CNO that predates the
	// observed-stable-generation annotation, seed the annotation on
	// pre-existing workloads whose rollout is already complete. Without
	// this, every workload looks "never observed stable" and any transient
	// pod unavailability triggers Progressing and eventually Degraded.
	// The migration runs once (the first SetFromPods call where
	// installComplete is true) so that workloads created after install
	// are correctly treated as new.
	if status.installComplete && !status.annotationMigrationDone {
		status.annotationMigrationDone = true
		for _, ds := range daemonSets {
			if ds.GetAnnotations()[names.ObservedStableGenerationAnnotation] == "" &&
		...
		for _, dep := range deployments {
			if dep.GetAnnotations()[names.ObservedStableGenerationAnnotation] == "" &&
				...
				if err := status.setAnnotation(context.TODO(), dep, names.ObservedStableGenerationAnnotation, &gen); err != nil {
					log.Printf("Error migrating Deployment %s/%s annotation: %v", dep.Namespace, dep.Name, err)
				}
			}
		}
		for _, ss := range statefulSets {
			if ss.GetAnnotations()[names.ObservedStableGenerationAnnotation] == "" &&
				...
				if err := status.setAnnotation(context.TODO(), ss, names.ObservedStableGenerationAnnotation, &gen); err != nil {
					log.Printf("Error migrating StatefulSet %s/%s annotation: %v", ss.Namespace, ss.Name, err)
				}
			}
		}
	}
