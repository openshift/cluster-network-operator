...
func (s *StatusManager) AddMachineConfigsWatcher(cfg *rest.Config, mgr manager.Manager) error {
	...
	err = c.Watch(&source.Informer{Informer: s.mcInformer}, handler.EnqueueRequestsFromMapFunc(enqueueRP))
	if err != nil {
		return err
	}

	return c.Watch(&source.Informer{Informer: s.mcpInformer}, handler.EnqueueRequestsFromMapFunc(enqueueRP))
}
