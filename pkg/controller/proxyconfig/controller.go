...
func (r *ReconcileProxyConfig) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	case request.Namespace != names.ADDL_TRUST_BUNDLE_CONFIGMAP_NS:
		...
		if err := r.client.Get(ctx, names.Proxy(), proxyConfig); err != nil {
			...
			return reconcile.Result{}, fmt.Errorf("failed to get proxy '%s': %w", names.PROXY_CONFIG, err)
		}
		...
		if err := r.client.Get(ctx, types.NamespacedName{Name: names.CLUSTER_CONFIG}, infraConfig); err != nil {
			...
			return reconcile.Result{}, fmt.Errorf("failed to get infrastructure config '%s': %w", names.CLUSTER_CONFIG, err)
		}
		if err := r.client.Get(ctx, types.NamespacedName{Name: names.CLUSTER_CONFIG}, netConfig); err != nil {
			...
			return reconcile.Result{}, fmt.Errorf("failed to get network config '%s': %w", names.CLUSTER_CONFIG, err)
		}
		...
		if err := r.syncProxyStatus(ctx, proxyConfig, infraConfig, netConfig, clusterConfig); err != nil {
			...
			return reconcile.Result{}, fmt.Errorf("failed to sync proxy '%s': %w", names.PROXY_CONFIG, err)
		}
...
func (r *ReconcileProxyConfig) generateSystemTrustBundle() (*corev1.ConfigMap, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to validate trust bundle %s: %w", names.SYSTEM_TRUST_BUNDLE, err)
	}
...
func (r *ReconcileProxyConfig) configMapIsProxyTrustedCA(ctx context.Context, cfgMapName string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get proxy '%s': %w", names.PROXY_CONFIG, err)
	}
