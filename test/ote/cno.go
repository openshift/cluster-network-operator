...
var _ = g.Describe("[sig-network] CNO", func() {
	...
	g.It("[JIRA:Networking][OTP] 72817-Make sure internalJoinSubnet and internalTransitSwitchSubnet is configurable post install as a Day 2 operation", g.Serial, func() {
		var (
			pod1Name          = "hello-pod1"
			pod2Name          = "hello-pod2"
			podLabel          = "hello-pod"
		...
		applyPatchWithCleanup := func(customPatch, originalPatch string) {
			...
			err := checkOVNKState(oc)
			o.Expect(err).NotTo(o.HaveOccurred(), "OVNkube didn't rollout successfully after applying patch")
		}
	...
	g.It("[JIRA:Networking][OTP] 51727-ovsdb-server and northd should not core dump on node restart", g.Serial, func() {
	...
	g.It("[JIRA:Networking][OTP] 72028-Join switch IP and management port IP for newly added node should be synced correctly into NBDB, pod on new node can communicate with old pod on old node", g.Serial, func() {
		ipStackType := checkIPStackType(oc)
		o.Expect(ipStackType).NotTo(o.BeEmpty())
		...
		g.By("Check host network addresses for the newly added node in northdb")
		...
		if ipStackType == "dualstack" || ipStackType == "ipv4single" {
			hostNetworkIPsv4 := getHostNetworkIPsinNBDB(oc, newNodeName, "v4")
			e2e.Logf("Got hostNetworkIPsv4 for node %s : %v", newNodeName, hostNetworkIPsv4)
			o.Expect(contains(hostNetworkIPsv4, nodeOVNK8sMgmtIPv4)).Should(o.BeTrue(), fmt.Sprintf("New node's mgmt IPv4 is not updated in node %s NBDB!", newNodeName))
			o.Expect(unorderedContains(hostNetworkIPsv4, joinSwitchIPv4)).Should(o.BeTrue(), fmt.Sprintf("New node's join switch IPv4 is not updated in node %s NBDB!", newNodeName))
		}
		if ipStackType == "dualstack" || ipStackType == "ipv6single" {
			hostNetworkIPsv6 := getHostNetworkIPsinNBDB(oc, newNodeName, "v6")
			e2e.Logf("Got hostNetworkIPsv6 for node %s : %v", newNodeName, hostNetworkIPsv6)
			o.Expect(contains(hostNetworkIPsv6, nodeOVNK8sMgmtIPv6)).Should(o.BeTrue(), fmt.Sprintf("New node's mgmt IPv6 is not updated in node %s NBDB!", newNodeName))
			o.Expect(unorderedContains(hostNetworkIPsv6, joinSwitchIPv6)).Should(o.BeTrue(), fmt.Sprintf("New node's join switch IPv6 is not updated in node %s NBDB!", newNodeName))
		}
