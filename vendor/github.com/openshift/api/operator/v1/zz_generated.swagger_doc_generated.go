...
var map_GatewayConfig = map[string]string{
	...
	"allowNoUplink":  "allowNoUplink allows the external gateway bridge (br-ex) to start in local gateway mode when it has no physical uplink port. When set to true, ovn-kubernetes will not require an uplink on the gateway bridge. When omitted or set to false, ovn-kubernetes requires an uplink (the current behavior). This setting only takes effect when routingViaHost is true (local gateway mode).",
